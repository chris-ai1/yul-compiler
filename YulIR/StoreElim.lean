...
`unusedStoreEliminator` / `redundantStoreEliminator` in the *overwritten-store* direction.

## What is removed

A store `D-store(loc, val)` is removed when, on **every** forward path, `loc` is **overwritten
by a later `D`-store to the provably-same location** before any operation can **observe domain
`D`** — no read of `D`, and no call/create/… that could observe it. The later store then makes
this one's value unobservable, so dropping it changes nothing observable (only gas, downward).

This is deliberately restricted to *overwrite*-based elimination (a subsequent store to the same
slot), for all three domains. In particular it does **not** try to remove a trailing memory store
just because the frame discards memory at the end: that would change `msize` / the active-word
count, which the round-trip correctness fingerprint (`YulIR.Check`) samples directly. Because the
killing store is to the *same* offset, memory expansion and final contents are preserved exactly,
so the rewrite is observationally identical, not merely "identical modulo msize".
...
a literal, or a variable that is never reassigned anywhere in the program (`mutatedVars`). Running
...
A backward pass mirroring `YulIR.DeadStore`'s liveness, but computing, per domain, the set of
locations *known to be overwritten before observed downstream* ("clobbered"):
...
* a **terminator** (`stop`/`invalid`/`return`/`revert`/`selfdestruct`) resets to the empty
  clobber state: control leaves, storage/transient persist (observable) and returned memory is read;
...
* **loops** and function bodies are analysed with an empty incoming clobber state
  (conservative: a store before a loop, or spanning a call boundary, is never assumed dead), so only
  local overwrites inside them are caught.
...
/-- The three store domains this pass tracks. Storage and transient are keyed exactly; memory is
byte-addressed but only ever eliminated via a same-offset overwrite, so it is tracked the same way. -/
inductive Dom
...
/-- Intersect a non-empty list of clobber states. -/
